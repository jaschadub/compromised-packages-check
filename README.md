...
| Woodpecker PyPI infostealer campaign — June 6 2026 (OSV MAL-2026-5274 through MAL-2026-5279) | Six legitimate scientific/systems packages with specific versions compromised: `dynamo-release` 1.5.4; `napari-ufish` 0.0.2, 0.0.3; `nucbox` 0.1.2, 0.1.3; `pantheon-toolsets` 0.5.5, 0.5.6; `spateo-release` 1.1.2; `uprobe` 0.1.3, 0.1.4 — each ships a heavily obfuscated JS payload executed via Bun runtime on Python startup; collects API keys, registry credentials, crypto wallet keystores, and password-manager data; exfiltrates via GitHub; attempts persistence and self-propagation by republishing infected packages using stolen credentials; related to the Mini Shai Hulud campaign; detected by kam193/bad-packages.eu |
